...
ResetDB.get('/1', (req, res) => {
  ...
  db.query(sql);
  sql = "CREATE TABLE IF NOT EXISTS log (id int AUTO_INCREMENT, user_id VARCHAR(50), content VARCHAR(255), PRIMARY KEY(id))";
  db.query(sql);
...
ResetDB.get('/2', (req, res) => {
  ...
  db.query(sql);
  sql = "CREATE TABLE log (id int AUTO_INCREMENT, user_id VARCHAR(50), content VARCHAR(255), PRIMARY KEY(id))";
  db.query(sql);
  sql = "INSERT INTO user (user_id, user_pw) VALUES('root', 'root')"
  db.query(sql);
  res.send('success');
